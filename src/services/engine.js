...
function AIEngine() {
  ...
  function getPerceptron(mode) {
    ...
    if (mode == "alpha") {
      var data = fs.readFileSync(
        process.cwd() + `/src/data/train-${mode}.data`
      );
      if (data.toString()) {
  ...
  function recordFeedbackAlpha(pText, pActual, pData, userId) {
    ...
    console.log(
      "input: ",
      pData.split("|").map((item) => +item),
      "confidenceList",
      getAlphaConfidenceList(pActual)
    );
    myTrainer.train([
      {
        input: pData.split("|").map((item) => +item),
        output: getAlphaConfidenceList(pActual),
      },
    ]);
  ...
  function recordFeedbackNumber(pText, pActual, pData, userId) {
    ...
    console.log(
      ...
      pData.split("|").map((item) => +item),
      "confidenceList",
      getNumberConfidenceList(pActual)
    );
    myTrainer.train([
      {
        input: pData.split("|").map((item) => +item),
        output: getNumberConfidenceList(pActual),
      },
    ]);
  ...
  return {
    ...
    feedback: function (mode, feedback, userId) {
      ...
      const pData = Buffer.from(
        decodeURIComponent(feedback[2] || "") || "",
        "base64"
      ).toString("ascii");
      if (mode == "alpha") {
